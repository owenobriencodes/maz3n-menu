...
namespace iiMenu.Mods
{
    public static class Overpowered
    {
        ...
        public static void GuardianCrashAll()
        {
            ...
            {
                crashAllDelay = Time.time + 0.1f;
                foreach (var rig in VRRigCache.AllRigs.Where(rig => !rig.isLocal && rig.transform.position.x < -5))
                {
        ...
        {
            VRRig.LocalRig.enabled = true;
            foreach (var rig in VRRigCache.AllRigs.Where(rig => !rig.isLocal).Where(rig => rig.leftHandLink.grabbedPlayer == NetworkSystem.Instance.LocalPlayer || rig.rightHandLink.grabbedPlayer == NetworkSystem.Instance.LocalPlayer))
            {
                VRRig.LocalRig.enabled = false;
        ...
        {
            foreach (VRRig rig1 in VRRigCache.AllRigs)
            {
                if (Time.time < GetBoxingDelay(rig1))
                    continue;

                foreach (VRRig rig2 in VRRigCache.AllRigs)
                {
        ...
        public static void OrbitAll()
        {
            ...
            if (rightTrigger > 0.5f && Time.time > flingDelay)
            {
                ...
                VRRig[] rigs = VRRigCache.AllRigs.Where(rig => !rig.isLocal).ToArray();
        ...
        public static void GiveFlyAll()
        {
            ...
            {
                thingdeb = Time.time + 0.1f;
                foreach (var plr in VRRigCache.AllRigs.Where(plr => !plr.isLocal).Where(plr => plr.rightThumb.calcT > 0.5f))
                {
        ...
        public static void Boxing()
        {
            foreach (VRRig rig1 in VRRigCache.AllRigs)
            {
                ...
                foreach (var targetDirection in from rig2 in VRRigCache.AllRigs where rig2 != rig1 where Vector3.Distance(rig2.leftHandTransform.position, rig1.headMesh.transform.position) < 0.25f || Vector3.Distance(rig2.rightHandTransform.position, rig1.headMesh.transform.position) < 0.25f select (rig1.headMesh.transform.position - rig2.headMesh.transform.position) * 20f)
                {
        ...
        public static void BarrelFlingAll()
        {
            ...
            foreach (var TargetRig in VRRigCache.AllRigs.Where(TargetRig => !TargetRig.IsTagged()))
            {
        ...
        public static void BarrelKickAll()
        {
            ...
            foreach (VRRig TargetRig in VRRigCache.AllRigs)
            {
        ...
        public static void StumpKickAll()
        {
            if (PhotonNetwork.InRoom)
            {
                ...
                CoroutineManager.instance.StartCoroutine(StumpKickDelay(() =>
                {
                    ...
                    foreach (VRRig rig in VRRigCache.AllRigs.Where(rig => !rig.IsLocal() && GorillaComputer.instance.friendJoinCollider.playerIDsCurrentlyTouching.Contains(rig.GetPlayer().UserId)))
        ...
        public static void SpazGrabbedRopes()
        {
            ...
            {
                RopeDelay = Time.time + 0.125f;
                VRRig randomRig = VRRigCache.AllRigs
