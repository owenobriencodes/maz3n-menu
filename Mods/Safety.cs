    ...
    public static class Safety
    {
        ...
        public static void AntiReport(Action<VRRig, Vector3> onReport)
        {
            ...
            foreach (GorillaPlayerScoreboardLine line in GorillaScoreboardTotalUpdater.allScoreboardLines)
            {
                ...
                foreach (var vrrig in from vrrig in VRRigCache.AllRigs where !vrrig.isLocal where OverlappingButton(vrrig, report.position) || (antiMute && OverlappingButton(vrrig, line.muteButton.gameObject.transform.position)) where !smartAntiReport || SmartAntiReport(line.linePlayer) select vrrig)
        ...
        {
            foreach (var vrrig in VRRigCache.AllRigs.Where(vrrig => !vrrig.isOfflineVRRig && vrrig.RawCosmeticString().Contains("LBAAK") || vrrig.RawCosmeticString().Contains("LBAAD") || vrrig.RawCosmeticString().Contains("LMAPY")))
            {
                try
                {
                    ...
                    if (player != null)
                    {
                        ...
                        text += "\n====================================\n";
                        text += "Text file generated with MAZ3N Menu";
        ...
        {
            foreach (var vrrig in VRRigCache.AllRigs.Where(vrrig => !vrrig.isOfflineVRRig && Visuals.specialCosmetics.Keys.Any(x => vrrig.RawCosmeticString().Contains(x))))
            {
                try
                {
        ...
        public static void ColorSpoof()
        {
            ...
            foreach (var rig in VRRigCache.AllRigs.Where(rig => !rig.isLocal).Where(rig => !colorSpoofRigs.Contains(rig)))
            {
                GorillaTagger.Instance.myVRRig.SendRPC("RPC_InitializeNoobMaterial", GetPlayerFromVRRig(rig), Random.Range(0f, 1f), Random.Range(0f, 1f), Random.Range(0f, 1f));
