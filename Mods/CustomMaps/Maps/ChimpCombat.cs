...
namespace iiMenu.Mods.CustomMaps.Maps
{
    public class ChimpCombat : CustomMap
    {
        ...
        public static void CrashOnTouch()
        {
            if (Time.time < crashDelay)
                return;
            foreach (var netPlayer in from rig in VRRigCache.AllRigs where !rig.isLocal && (Vector3.Distance(GorillaTagger.Instance.leftHandTransform.position, rig.headMesh.transform.position) < 0.25f || Vector3.Distance(GorillaTagger.Instance.rightHandTransform.position, rig.headMesh.transform.position) < 0.25f) select GetPlayerFromVRRig(rig))
            {
